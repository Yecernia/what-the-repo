...
This is the **source code of the hosted what-the-repo Web product**: frontend, backend, repository analysis, agents, tests and deployment configuration. You do not need to clone this repository or install Docker to try the product online.

To explore the implementation, contribute, or study deployment, see the [development guide](DEVELOPMENT.md) and [contribution guide](CONTRIBUTING.md). A separate login-free local edition or desktop client has not been released.
