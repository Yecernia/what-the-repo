...
The root MIT license does not replace upstream terms. Preserve the collected
notices with any redistributed copies. README acknowledgements alone do not
replace copyright notices or full license texts where those are required.

## Package notice exceptions
...
license/notice. The following 13 records deliberately remain marked
`metadata-only-or-partial`:

| Packages | Evidence / limitation |
| --- | --- |
| `@mariozechner/clipboard@0.3.9` and ten platform packages at `0.3.9` | Published metadata declares MIT. The inspected package/upstream tree does not provide a full LICENSE file. The fork credits CrossCopy/clipboard and clipboard-rs; native Rust dependencies require their own binary-level inventory. No replacement copyright line has been invented. |
...
These are explicit redistribution follow-ups, not missing entries silently
reported as complete. Merely declaring npm dependencies in this source release
does not include their binaries. Before publishing a preinstalled image,
...
`node scripts/check-license-inventory.mjs --distribution` intentionally fails
while these exception records remain.

## Licenses requiring particular attention

- `lightningcss@1.33.0` and its eleven platform packages declare MPL-2.0.
  They are Web development/build dependencies. Do not relicense their files as
  MIT. If distributing covered executables or modified covered files, satisfy
...
- `caniuse-lite@1.0.30001809` contains browser-support data under CC-BY-4.0.
  ...
  dataset. This source snapshot declares the unchanged dependency; it does not
  copy or modify the dataset.
...
- Apache-2.0 dependencies retain their full LICENSE and any supplied NOTICE;
  modification notices must accompany future copied/modified source.
...
The repository publishes build/configuration recipes, not copies of the following
upstream images or installed programs. Their licenses are separate from npm:

| Component | Usage / license boundary |
| --- | --- |
...
| Redis 7.4 (`redis:7.4-alpine`) | External service image. This series uses **RSALv2 or SSPLv1**, not the older BSD license. Its terms are not replaced by what-the-repo MIT. See [Redis's version-specific license table](https://redis.io/legal/licenses/). No version or infrastructure replacement is made in this licensing task. |
...
This inventory does not claim that container images, native packages or a future
desktop distribution have already passed a complete redistribution review.
